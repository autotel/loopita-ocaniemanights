...
Looper=function(){
  ...
  this.components={
    play:{},
    stop:new Button(this.$.controlPanel,{label:"■"}),
...
Looper.prototype.$appendTo=function($who){
  if($who.length>0){
    if(this.appended){
...
Looper.prototype.setSampler=function(databaseItem){
  ...
  if(databaseItem.hasOwnProperty("source")){
    ...
    this.engine=new Tone.Player(databaseItem.source,function(a){
      ...
      if(me.components.play) me.components.play=new Button(me.$.controlPanel,{label:"►"}).onClick(function(){
          me.metroLoop();
        });
...
Looper.prototype.tempoSync=function(factor){
...
Looper.prototype.tempoFactor=function(factor){
  this.playFactor*=factor;
  this.playbackRate*=factor;
  this.engine.playbackRate=this.playbackRate;
  this.$.playRatio.html("R:"+this.playbackRate);
}
Looper.prototype.resetRate=function(rate){
  if(!rate)
    rate=1;
